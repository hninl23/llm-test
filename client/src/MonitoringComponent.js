...
const MonitoringComponent = ({ tokenCountsHistory }) => {
    ...
    const [limits, setLimits] = useState({
        'gpt-3.5-turbo': 40000,
        //add others
        'text-embedding-ada-002(free)': 150000,
        'text-embedding-ada-002-T1(T1)': 1000000,
        'gpt-4': 10000
    })
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        const fetchAPICallData = async () => {
          try {
            const apiCallResponse = await fetch('http://127.0.0.1:5000/get_api_calls');
            const apiCallData = await apiCallResponse.json();
            console.log('API Calls', apiCallData);
    
            const newCallData = Object.entries(apiCallData.total_api_calls).map(([model, count]) => ({ model, count }));
  ...
  return (
    <div>
      ...
      <div className='ans-container'>
        ...
        <select id='modelDropDown' value={selectedModel} onChange={handleModelChange}>
            ...
            <option value="text-embedding-ada-002(free)">TEXT-EMBEDDING-ADA-002(FREE)</option>
            <option value="text-embedding-ada-002-T1(T1)">TEXT-EMBEDDING-ADA-002(TIER 1)</option>
        </select>
    ...
    <p className='title-mon'>Total API Calls (in day):</p>
    <ul>
      {currentCall.map((entry, index) => (
        <li key={index}>
